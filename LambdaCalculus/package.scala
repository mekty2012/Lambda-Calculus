...
package object LambdaCalculus extends Language {
  ...
  def resolveRedex(l:LambdaCalculus): LambdaCalculus = l match {
    case App(f, s) => f match {
      ...
      case _ => throw new LanguageException("Expression is not a redex")
    }
    case _ => throw new LanguageException("Expression is not a redex")
  }
  ...
  def normalOrderEvaluation(l: LambdaCalculus): Lambda = {
    if (!isClosed(l)) {
      throw new LanguageException("Expression is not closed")
    }
    l match {
      case Id(n) => throw new LanguageException("Never happens")
      case App(f, s) => {
  ...
  def eagerEvaluation(l:LambdaCalculus) : Lambda = {
    ...
    l match {
      case Id(_) => throw new LanguageException("Expression is not closed")
      case App(f, s) => {
  ...
  object ChurchEncoding {
    ...
    def num(n:Int) : LambdaCalculus =  {
      if (n < 0) {
        throw new LanguageException("Church Encoding only accepts natural number")
      }
  ...
  object LambdaCalculusParser extends RegexParsers {
    ...
    str ^^ (s => Id(s))
    def apply(str:String):LambdaCalculus = parseAll(expr, str).getOrElse(throw new LanguageException("Bad Syntax"))
  }
}
